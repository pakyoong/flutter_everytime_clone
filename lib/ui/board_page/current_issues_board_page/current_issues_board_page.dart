...
import 'package:everytime/ui/board_page/current_issues_board_page/current_issues_board_detail_page.dart';
import 'package:everytime/ui/board_page/current_issues_board_page/current_issues_board_write_page.dart';
import 'package:flutter/material.dart';
import 'package:everytime/bloc/board_page/post_bloc.dart';
import 'package:cloud_firestore/cloud_firestore.dart';

class CurrentIssuesBoard extends StatefulWidget {
  final PostBloc BoardBloc;
  const CurrentIssuesBoard({super.key, required this.BoardBloc});

  @override
  State<CurrentIssuesBoard> createState() => _CurrentIssuesBoardState();
}

class _CurrentIssuesBoardState extends State<CurrentIssuesBoard> {
  late PostBloc current_issuesBoardBloc= PostBloc();
  List<Post> postList = [];
          ...
          IconButton(
            ...
            onPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (context) => CurrentIssuesBoard(
                          BoardBloc: current_issuesBoardBloc,
                        )), //검색 기능 일단 없음
              );
            },
          ),
            ...
            onSelected: (int item) async {
              if (item == 1) {
                ...
                Navigator.push(
                  context,
                  MaterialPageRoute(
                      builder: (context) => CurrentIssuesBoardWrite(
                            current_issuesBoardBloc: current_issuesBoardBloc,
                          )),
                );
              } else if (item == 3) {
                // 즐겨찾기 기능 보류
