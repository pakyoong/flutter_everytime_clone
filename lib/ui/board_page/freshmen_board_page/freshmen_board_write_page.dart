...
class FreshmenBoardWrite extends StatefulWidget {
  final PostBloc freshmenBoardBloc;
  const FreshmenBoardWrite({super.key, required this.freshmenBoardBloc});

  @override
  State<FreshmenBoardWrite> createState() => FreshmenBoardWriteState();
}

class FreshmenBoardWriteState extends State<FreshmenBoardWrite> {
  late PostBloc freshmenBoardWriteBloc=PostBloc();
  ...
  void initState() {
    super.initState();
    freshmenBoardWriteBloc = PostBloc();
    // _titleController.text = widget.freshmenBoardWriteBloc.title;
    // _contentController.text = widget.freshmenBoardWriteBloc.content;
    isAnonym = widget.freshmenBoardBloc.isAnonymous;
  }
            ...
            child: ElevatedButton(
              onPressed: () {
                ...
                if (title.trim().isEmpty || content.trim().isEmpty) {
                  ...
                  // FreshmenBoardWriteBloc에 제목과 내용 업데이트
                  widget.freshmenBoardBloc.updateTitle(title);
                  widget.freshmenBoardBloc.updateContent(content);

                  widget.freshmenBoardBloc.submitPost(boardId).then((_) {
                    // 글이 추가된 후에 새로고침
                    Navigator.pop(context);
                    Navigator.pushReplacement(
                      ...
                      MaterialPageRoute(
                        builder: (context) =>
                            FreshmenBoard(BoardBloc: widget.freshmenBoardBloc),
                      ),
                    );
