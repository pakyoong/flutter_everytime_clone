...
class SecretBoardWrite extends StatefulWidget {
  final PostBloc secretBoardBloc;
  const SecretBoardWrite({super.key, required this.secretBoardBloc});
...
class SecretBoardWriteState extends State<SecretBoardWrite> {
  late PostBloc secretBoardWriteBloc = PostBloc();
  ...
  void initState() {
    super.initState();
    secretBoardWriteBloc = PostBloc();
    // _titleController.text = widget.secretBoardWriteBloc.title;
    // _contentController.text = widget.secretBoardWriteBloc.content;
    isAnonym = widget.secretBoardBloc.isAnonymous;
  }
            ...
            child: ElevatedButton(
              onPressed: () {
                ...
                if (title.trim().isEmpty || content.trim().isEmpty) {
                  ...
                  // SecretBoardWriteBloc에 제목과 내용 업데이트
                  widget.secretBoardBloc.updateTitle(title);
                  widget.secretBoardBloc.updateContent(content);

                  widget.secretBoardBloc.submitPost(boardId).then((_) {
                    // 글이 추가된 후에 새로고침
                    Navigator.pop(context);
                    Navigator.pushReplacement(
                      ...
                      MaterialPageRoute(
                        builder: (context) =>
                            SecretBoard(BoardBloc: widget.secretBoardBloc),
                      ),
                    );
