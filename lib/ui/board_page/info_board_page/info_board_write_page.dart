...
class InfoBoardWrite extends StatefulWidget {
  final PostBloc infoBoardBloc;
  const InfoBoardWrite({super.key, required this.infoBoardBloc});

  @override
  State<InfoBoardWrite> createState() => InfoBoardWriteState();
}

class InfoBoardWriteState extends State<InfoBoardWrite> {
  late PostBloc infoBoardWriteBloc=PostBloc();
  ...
  void initState() {
    super.initState();
    infoBoardWriteBloc = PostBloc();
    // _titleController.text = widget.infoBoardWriteBloc.title;
    // _contentController.text = widget.infoBoardWriteBloc.content;
    isAnonym = widget.infoBoardBloc.isAnonymous;
  }
            ...
            child: ElevatedButton(
              onPressed: () {
                ...
                if (title.trim().isEmpty || content.trim().isEmpty) {
                  ...
                  // InfoBoardWriteBloc에 제목과 내용 업데이트
                  widget.infoBoardBloc.updateTitle(title);
                  widget.infoBoardBloc.updateContent(content);

                  widget.infoBoardBloc.submitPost(boardId).then((_) {
                    // 글이 추가된 후에 새로고침
                    Navigator.pop(context);
                    Navigator.pushReplacement(
                      ...
                      MaterialPageRoute(
                        builder: (context) =>
                            InfoBoard(BoardBloc: widget.infoBoardBloc),
                      ),
                    );
