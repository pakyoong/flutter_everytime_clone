...
import 'package:everytime/ui/board_page/graduates_board_page/graduates_comment_detail.dart';
import 'package:flutter/material.dart';

class GraduatesBoardDetail extends StatefulWidget {
  final Post post;

  const GraduatesBoardDetail({super.key, required this.post});

  @override
  State<GraduatesBoardDetail> createState() => _GraduatesBoardDetailState(post: post);
}

class _GraduatesBoardDetailState extends State<GraduatesBoardDetail> {
  late Post post;
  late String postId;
  late int like;
  ...
  PostBloc graduatesBoardBloc = PostBloc();
  final TextEditingController _commentController = TextEditingController();

  _GraduatesBoardDetailState({required this.post}) {
    postId = post.postId;
    like = post.like;
  }
            ...
            return AlertDialog(
              ...
              actions: [
                ...
                ElevatedButton(
                  ...
                  onPressed: () async {
                    await graduatesBoardBloc.postLike(boardId, post.postId);
                    setState(() {
                      post.like++;
                    });
                    Navigator.pop(context);
        ...
        leading: IconButton(
          ...
          onPressed: () {
            ...
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => GraduatesBoard(boardBloc: graduatesBoardBloc),
              ),
            );
          },
        ),
            ...
            onSelected: (int item) async {
              ...
              if (item == 1) {
                Navigator.pushReplacement(
                  currentContext,
                  MaterialPageRoute(
                    builder: (currentContext) => GraduatesBoardDetail(post: post),
                  ),
                );
              }
              if (item == 2) {
              ...
              if (item == 3) {
                try {
                  await graduatesBoardBloc.deletePost(boardId, post.postId);
                  Navigator.pop(currentContext);
                  Navigator.pushReplacement(
                    currentContext,
                    MaterialPageRoute(
                      builder: (currentContext) =>
                          GraduatesBoard(boardBloc: graduatesBoardBloc),
                    ),
                  );
                } catch (error) {
                  showDialog(
                            ...
                            if (comment.trim().isEmpty) {
                              ...
                              commentBloc
                                ...
                                Navigator.pushReplacement(
                                  ...
                                  MaterialPageRoute(
                                    builder: (context) =>
                                        GraduatesBoardDetail(post: post),
                                  ),
                                );
