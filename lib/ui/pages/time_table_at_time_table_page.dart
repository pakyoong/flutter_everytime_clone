...
class TimeTableAtTimeTablePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder(
      stream: userBloc.selectedClassTimetable,
  ...
  Widget _buildTimeTableEmpty(BuildContext context) {
    return SizedBox(
      height: availableHeight * 0.485,
      child: EmptyContent(
        icon: Icons.table_chart_outlined,
        title: '이번 학기 시간표를 만들어주세요',
        contentIsString: false,
        content: Container(
          height: availableHeight * 0.045,
          width: availableWidth * 0.325,
          margin: EdgeInsets.only(
            top: availableHeight * 0.02,
          ),
          decoration: BoxDecoration(
            borderRadius: BorderRadius.circular(availableHeight * 0.02775),
  ...
  void _addNewTimeTable() {
    ClassTimetable newTimeTable = ClassTimetable(
    ...
    userBloc.updateSelectedClassTimetable(newTimeTable);
  }
      ...
      builder: (_, timeListSnapshot) {
        if (timeListSnapshot.hasData) {
          return CustomContainer(
            height:
                availableHeight * (0.0579 * timeListSnapshot.data!.length + 0.025),
            usePadding: false,
            child: StreamBuilder(
              ...
              builder: (_, dayOfWeekSnapshot) {
                if (dayOfWeekSnapshot.hasData) {
                  return StreamBuilder(
                    stream: userBloc.currentSelectedClassTimetable!.classTimetableData,
                    builder: (_, timeTableDataSnapshot) {
                      if (timeTableDataSnapshot.hasData) {
                        return TimeTableChart(
                          userBloc: userBloc,
                          classTimetable: timeTableDataSnapshot.data!,
